...
static void ompt_tsan_thread_begin(ompt_thread_t thread_type,
  ...
  if(!thread_local_clock)
    thread_local_clock =
      new THREAD_CLOCK(my_next_id(), 0, true);
...
static void ompt_tsan_parallel_begin(ompt_data_t *parent_task_data,
  ...
  // end of computation
  thread_local_clock->Stop(OMP, "ParallelBegin");
...
static void ompt_tsan_parallel_end(ompt_data_t *parallel_data,
  ...
  //OmpHappensAfter(Data->GetBarrierPtr((Data->BarrierIndex+2)%3));
  //OmpHappensAfter(Data->GetBarrierPtr(1));
  // end of parallel region: get the max computation of all threads
  // double maxUseful = 0;
  /*	std::string res = "\n\033[1;36m---Useful computation of threads of proc
     " + std::to_string(myProcId) + "--\033[0m\n"; for (int i = 0; i <
     thread_clocks.size(); i++) { double currUsefulTime =
     thread_clocks[i]->useful_computation_critical; res += "Thread_" +
     std::to_string(i) + ": " + std::to_string(currUsefulTime) + "s; " +
     std::to_string(thread_clocks[i]->useful_computation_thread) + "s\n";
          }
          res +=
     "\033[1;36m--------------------------------------------\033[0m\n";*/

  //	crit_path_useful_time += maxUseful;

  //	std::cout << res;
  // start of computation
  thread_local_clock->Start(OMP, "ParallelEnd");
//  startClock(thread_local_clock);
...
static void ompt_tsan_implicit_task(ompt_scope_endpoint_t endpoint,
  ...
  case ompt_scope_begin:
    if (type & ompt_task_initial) {
      ...
      // startClock(thread_local_clock, true);
    } else {
      if(thread_local_clock->stopped_mpi_clock){
        thread_local_clock->Start(MPI_ONLY, __func__);
      }
      OmpHappensAfter(ToParallelData(parallel_data)->GetParallelPtr());
    }
    task_data->ptr = TaskData::New(ToParallelData(parallel_data), thread_num, type);

    if (!(type & ompt_task_initial) || thread_local_clock->stopped_clock)
    {
      if (thread_local_clock->stopped_mpi_clock) {
        thread_local_clock->Start(ALL, __func__);
      } else {
        thread_local_clock->Start(OMP, __func__);
      }
    }
...
/*    if (!(type & ompt_task_initial))
      thread_local_clock->Stop(OMP, "ImplicitTaskEnd");*/
    ...
    if(team_size==1 && ! (type & ompt_task_initial))
      thread_local_clock->Stop(OMP, __func__);
      
    assert(thread_local_clock->stopped_clock == true);
    
    /*if (analysis_flags->stopped == 0)
      printf("Thread time: useful %lf, critical %lf\n",
...
static void ompt_tsan_sync_region(ompt_sync_region_t kind,
  ...
  case ompt_scope_beginend:
    ...
    Data->InBarrier = true;
    thread_local_clock->Stop(OMP, "SyncRegionBegin");
    switch (kind) {
  ...
  case ompt_scope_end:
    ...
    if(parallel_data)
      thread_local_clock->Start(OMP, "SyncRegionEnd");
//      startClock(thread_local_clock);
    break;
  }
}
...
static void ompt_tsan_task_schedule(ompt_data_t *first_task_data,
  ...
  if (!FromTask->InBarrier) {
    thread_local_clock->Stop(OMP, "TaskEnd");
  }
  ...
  if (!ToTask->InBarrier) {
    thread_local_clock->Start(OMP, "TaskBegin");
//    startClock(thread_local_clock);
  }
}
...
static void ompt_tsan_mutex_acquire(ompt_mutex_t kind,
                                              unsigned int hint,
                                              unsigned int impl,
                                              ompt_wait_id_t wait_id,
                                              const void *codeptr_ra){
  thread_local_clock->Stop(OMP, "MutexAcquire");
