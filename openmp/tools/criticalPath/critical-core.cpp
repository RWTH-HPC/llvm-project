...
double getTime() {
  ...
  return curr.tv_sec + curr.tv_nsec * 1e-9;
}
...
/*    assert(thread_clock->stopped_omp_clock == false);
    thread_clock->stopped_omp_clock = true;
    thread_clock->outsideomp_proc=thread_clock->outsideomp_critical=thread_clock->outsideomp_critical_nooffset=thread_clock->outsideomp_thread=0;*/
/*    if (thread_clock->stopped_omp_clock == true) {
      thread_clock->stopped_omp_clock = false;
      thread_clock->outsideomp_proc=thread_clock->outsideomp_critical=thread_clock->outsideomp_critical_nooffset=thread_clock->outsideomp_thread=-getTime();
    }*/
  }
}
...
//  startProgrammTime = std::chrono::steady_clock::now();
//  double currTime = std::chrono::duration_cast<std::chrono::duration<double>>(
...
//  endProgrammTime = std::chrono::steady_clock::now();
...
void finishMeasurement() {
  ...
  if (thread_local_clock->stopped_omp_clock == false)
    thread_local_clock->Stop(endProgrammTime, OMP_ONLY, __func__);

  double totalRuntimeReal = endProgrammTime - startProgrammTime;
/*        std::chrono::duration_cast<std::chrono::duration<double>>(
            endProgrammTime - startProgrammTime)
            .count();*/
//  if(thread_local_clock->stopped_omp_clock == false)
//    stopOmpClock(thread_local_clock);
  ...
  //    assert(thread_clocks[i]->stopped_mpi_clock == true);
    ...
    num_threads = 1;
    uc_max[0] = uc_avg[0] = thread_local_clock->useful_computation_thread.load();
    uc_max[2] = uc_avg[2] = thread_local_clock->outsideomp_thread.load();
  }
//  uc_max[1] = uc_avg[1] = thread_local_clock->outsidempi_thread.load();
  ...
  PMPI_Reduce(&uc_max, &maxComputation, NUM_SHARED_METRICS, MPI_DOUBLE, MPI_MAX, 0,
              MPI_COMM_WORLD);
  PMPI_Reduce(&uc_avg, &avgComputation, NUM_SHARED_METRICS, MPI_DOUBLE, MPI_SUM, 0,
              MPI_COMM_WORLD);
  PMPI_Reduce(&num_threads, &total_threads, 1, MPI_INT, MPI_SUM, 0,
              MPI_COMM_WORLD);
  PMPI_Reduce(&proc_counts, &total_counts, sizeof(proc_counts)/sizeof(int), MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
...
/*  for (int i=0; i<NUM_SHARED_METRICS-2; i++){
    avgComputation[i] = avgComputation[i] / number_of_procs;
  }*/
...
/*    double mpiCommE = maxComputation[1] / totalRuntimeReal;
    double mpiTE = totalOutsideMPIIdeal / totalRuntimeReal;
    double mpiSerE = maxComputation[1] / totalOutsideMPIIdeal;
    double mpiLB = avgComputation[1] / maxComputation[1];
    double mpiPE = mpiLB * mpiCommE;

    double ompCommE = CommE / mpiCommE;
    double ompTE = TE / mpiTE;
    double ompSerE = SerE / mpiSerE;
    double ompLB = LB / mpiLB;
    double ompPE = PE / mpiPE;*/
    
/*    double ompCommE = maxComputation[2] / totalRuntimeReal;
    double ompTE = totalOutsideOMPIdeal / totalRuntimeReal;
    double ompSerE = maxComputation[2] / totalOutsideOMPIdeal;
    double ompLB = avgComputation[2] / maxComputation[2];
    double ompPE = ompLB * ompCommE;

    double mpiCommE = CommE / ompCommE;
    double mpiTE = TE / ompTE;
    double mpiSerE = SerE / ompSerE;
    double mpiLB = LB / ompLB;
    double mpiPE = PE / ompPE;*/

    // Computation[0] = avg/max_P avg_t useful_computation_thread;
    // Computation[1] = avg/max_P outsidempi_proc (crit.);
    // Computation[2] = avg/max_P avg_t outsideomp_thread;
    // Computation[3] = avg/max_P useful_computation_proc (crit.);

    // Computation[4] = avg/max_P useful_computation_proc (crit.);
    // Computation[5] = PL-runtime
    // Computation[6] = w-avg/max ( max_t(useful_computation_thread))
    // totalRuntimeReal = wallclock
    // totalRuntimeIdeal = critical Useful
    // totalOutsideOMPIdeal = critical outside OpenMP (UC + MPI)
    
//    double mpiLB = avgComputation[3] / maxComputation[3];
    double mpiLB = avgComputation[6] / maxComputation[0];
    double ompLB = LB / mpiLB;
    
//    double mpiCommE = avgComputation[5] / totalRuntimeReal;
    double ompTE = totalOutsideOMPIdeal / totalRuntimeReal;
    double mpiTE = TE / ompTE;

/*    double mpiCommE = maxComputation[3] / totalRuntimeReal;
    double ompCommE = CommE / mpiCommE;

    double ompSerE = ompCommE / ompTE;
    double mpiSerE = mpiCommE / mpiTE;*/
...
/*    pop_string +=
    ...
    pop_string +=
        "    MPI Communication Efficiency2:   " + std::to_string(mpiCommE2) +
        "\n";
    pop_string +=
...
/*    pop_string +=
    ...
    pop_string +=
        "    OMP Communication Efficiency2:   " + std::to_string(ompCommE2) +
        "\n";
    pop_string +=
...
/*  for (auto tc: thread_clocks)
  ...
  void SYNC_CLOCK::OmpHBefore() {
    ...
    update_maximum(outsideomp_critical_nooffset,
                   thread_local_clock->outsideomp_critical_nooffset.load());
    update_maximum(outsideomp_proc,
                   thread_local_clock->outsideomp_proc.load());
    update_maximum(outsidempi_proc,
                   thread_local_clock->outsidempi_proc.load());
  ...
  void SYNC_CLOCK::OmpHAfter() {
    ...
    update_maximum((thread_local_clock->outsideomp_proc),
                   outsideomp_proc.load());
    update_maximum((thread_local_clock->outsidempi_proc),
                   outsidempi_proc.load());
  }
...
//      assert(thread_local_clock->stopped_clock == false);
//      assert(thread_local_clock->stopped_mpi_clock == false);
//      assert(thread_local_clock->stopped_omp_clock == false);
    }
}
...
void stopTool(){
  if (analysis_flags->running) {
    printf("ending tool\n");
    double time = getTime();
    thread_local_clock->Stop(time, ALL, __func__);
    analysis_flags->running = false;
